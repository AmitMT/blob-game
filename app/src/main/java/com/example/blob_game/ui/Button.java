...
public class Button extends androidx.appcompat.widget.AppCompatTextView {
	...
	public Button(Context context, AttributeSet attributeSet) {
		...
		int defaultPadding = arr.hasValue(0) ?
			arr.getDimensionPixelOffset(0, -1) : (int) Utils.convertDpToPixel(10, context);
		int[] padding = {
			arr.hasValue(1) ?
				arr.getDimensionPixelOffset(1, -1) : defaultPadding * 2,
			arr.hasValue(2) ?
				arr.getDimensionPixelOffset(2, -1) : defaultPadding,
			arr.hasValue(3) ?
				arr.getDimensionPixelOffset(3, -1) : defaultPadding * 2,
			arr.hasValue(4) ?
